...
const CameraSection = () => {
  const CAROUSEL_IMAGES = [
    ...
    { src: "/assets/images/carousel_6.jpg", size: "2x", type: "망원 | 48mm" },
    {
      ...
      <div className="relative max-w-[860px] mx-auto flex flex-row w-full items-center justify-center">
        <div className="text-[19px]">
          <span className="text-white">
            {CAROUSEL_IMAGES[activeIndex].size}{" "}
          </span>
          ...
          <div className="flex flex-col flex-1 gap-4 items-start justify-center">
            <div className="flex flex-col gap-8 w-[330px]">
              ...
              <motion.div
                ...
                viewport={{ amount: "all" }}
              >
                향상된 성능은 인물 사진을 찍을 때 빛을 발합니다. 더 이상 인물
                사진 모드로 일일이 전환할 필요 없이, 피사체가 사람 혹은 강아지,
                고양이 같은 반려동물인 경우 iPhone이 자동으로 심도 정보를
                포착합니다. 그래서 촬영 중에{" "}
                <span className="text-[#f5f5f7]">
                  인물 사진처럼 흐림 효과가 아름답게 배경에 적용된 상태
                </span>
                를 확인하면서 찍을 수 있고, 촬영 후에 ‘사진’ 앱에서 인물
                사진으로 바꾸는 것도 가능하죠.
              </motion.div>
            </div>
          </div>
